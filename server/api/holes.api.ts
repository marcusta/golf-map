...
const UpdateHoleInput = Type.Object({
    ...
    strokeIndex: Type.Optional(Type.Union([Type.Number(), Type.Null()])),
    notes: Type.Optional(Type.String()),
    savedRegionJson: Type.Optional(Type.String()),
});
...
export function createHolesApi(svc: HolesService) {
    ...
    return {
        ...
        update:       { method: 'POST' as const, path: '/holes/update',            fn: (input: Static<typeof UpdateHoleInput>)        => svc.update(input.id, input.version, { par: input.par, strokeIndex: input.strokeIndex, notes: input.notes, savedRegionJson: input.savedRegionJson }), schema: UpdateHoleInput,        middleware: mw },
