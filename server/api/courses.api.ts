...
const UpdateCourseInput = Type.Object({
    ...
    homeLon: Type.Optional(Type.Number()),
    notes: Type.Optional(Type.String()),
    siteId: Type.Optional(Type.Union([Type.String(), Type.Null()])),
});
...
export function createCoursesApi(svc: CoursesService) {
    ...
    return {
        ...
        update:  { method: 'POST' as const, path: '/courses/update',  fn: (input: Static<typeof UpdateCourseInput>)  => svc.update(input.id, input.version, { name: input.name, crs: input.crs, georeferenceJson: input.georeferenceJson, homeLat: input.homeLat, homeLon: input.homeLon, notes: input.notes, siteId: input.siteId }), schema: UpdateCourseInput,  middleware: mw },
