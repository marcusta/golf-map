...
export class MapBuildClientService {
    ...
    /** Switch the active ortho vintage (server re-tiles) and begin polling. */
    async setOrtho(courseId: string, collection: string): Promise<void> {
        this.stop();
        const job = await request(this.starting, this.error, () => this.mapBuildApi.setOrtho({ courseId, collection }));
        if (!job) return;
        this.job.set(job);
        if (!isTerminal(job)) this.beginPolling(job.id);
    }
