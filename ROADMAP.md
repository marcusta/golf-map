...
Detailed kickoff: `docs/prompts/phase5-kickoff.md`. Server surface (schema `clubs`/`game_plans`/`game_plan_holes`/`plan_shots`, `game-plans`/`clubs` APIs + gen clients) already exists from Phase 0 — Phase 5 is gap-fill + shared math + web UI. Web + shared first; iOS planner deferred to a follow-on (a well-factored `shared/strategy/` makes it a port, not a redesign).
- **Shared math** — port v1 club/wind/dispersion math into `shared/strategy/` (pure TS, zero framework deps, tested against v1 numbers; the reference the Swift port later mirrors). Porting it finalizes the club data model (v1 export has 13 clubs + 18 plans as fixtures).
- **Dispersion zones** (v1 parity, core) — club dispersion ellipse (lateral × derived length, wind-adjusted), anchorable to any planning node: tee box, aim point, or plan shot point.
- **Corridor-width ruler** (new) — draggable ruler perpendicular to the aim line, reads in-play width in metres ("how much can I miss before trees/hazard/bunker/OOB — rough is fine"). A **persisted plan-attached entity** (new `plan_gates`: station + asymmetric half-widths + `version`), syncs + renders like `plan_shots`, visible in on-course playing mode. MVP = manual drag; entity/math shaped so an auto-computed width (ray-cast the perpendicular against `course_features` hazard/OOB boundaries, rough = in play) is a later add, not a remodel.
- **Web plan editor** — per tee, per hole, shot sequence, dispersion overlays, corridor rulers, wind scenarios, plays-like; on `@basics/core`, reusing the course-builder map + overlay + EntityStore patterns.
- **Schema gaps to settle** (data-model decisions, do up front): wind is per-plan today → decide per-hole / scenario granularity; units — store canonical m/s, mph is display-only; add `plan_gates`; confirm the plan shape carries everything the phone needs to render guidance.
...
**Decided (2026-07-05, Phase 5 kickoff)**
- **Wind granularity: per-hole override, no scenario table.** `game_plan_holes` gains nullable `wind_speed_mps`/`wind_direction_deg`; null = inherit plan-level wind. Effective wind = hole override ?? plan wind. A "scenario" is just editing the plan-level wind.
- **Units: m/s canonical end-to-end.** `shared/strategy/` wind API takes m/s and converts internally (exact v1 constant 2.23694) to apply the mph-based v1 curve; mph exists only as UI display/input conversion.
- **Club model unchanged.** v1 math needs only nominal carry + lateral dispersion; length dispersion (tiers >150 m: 8%, 100–150 incl.: 6%, <100: 5%) and ±5%/±4% bands are derived. `clubs.carry_m` + `dispersion_m` suffice — no migration.
- **`plan_gates` shape (corridor ruler):** `game_plan_hole_id` FK cascade, absolute WGS84 station `lat`/`lon` (survives shot edits, phone renders without leg recompute), `direction_deg` (corridor-axis bearing; ruler is its perpendicular — self-contained for the phone), asymmetric `half_width_left_m`/`half_width_right_m`, `source` `'manual'|'computed'` (auto ray-cast fill later is an UPDATE, not a remodel), `sort_order`, `version`. API: add/update/remove (version-checked), gates ride along in the `getByCourse` tree.
- **Plan-shape additions for on-course guidance:** `plan_shots.label` (nullable text, e.g. "layup left of bunker") and `game_plan_holes.notes` (nullable text). Same migration (003).
- **`reorderShots` stays version-unaware deliberately** (idempotent sort_order rewrite; per-row version checks would fail simple drags), but now validates the id set matches the hole's shots exactly.
- **Dispersion visual = ellipse** (kickoff mandate; v1's `DispersionEllipse` was dead code, arcs shipped — we implement the intended ellipse): anchored at any planning node, center = origin + wind-adjusted carry along bearing, full extents = derived length dispersion × lateral dispersion, rotated by bearing, built in EPSG:3006 and converted to WGS84 at render. Head/tail wind curve is the exact v1 rules (−1%/mph head, +0.5%/mph tail, 1.3%/0.34% above 18 mph total, strict >, discontinuity preserved).
- **Crosswind lateral drift is a documented v1.1 extension** (v1 computed the component but never consumed it; kickoff wants a lateral shift): drift = carry × crosswind_mph × 0.005, applied to the ellipse center perpendicular to bearing. Flagged for Phase 7 calibration.
- **Plays-like: reuse the existing simple caddie rule** (`horizontal + elevationΔ`) — `shared/strategy/` hosts the reference port of web `measure-state.ts` / iOS `PlaysLike.swift`; web planner samples elevation client-side via the existing `ElevationService` (Terrain-RGB z17 bilinear). No new server elevation endpoint.
- **Corridor ray-cast hazard set is a parameter**, default {bunker, water, water_creek, outside, deep_rough, trees, penalty_yellow, penalty_red, oob}; fairway/green/tee/semi_rough/rough/path are in-play. Math is pure in `shared/strategy/`, operating on flattened EPSG:3006 rings.
